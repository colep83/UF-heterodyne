# save_phase_maps.py saves a series of phase images to the computer
import os
import PySpin
import numpy
...
parser.add_argument('-f', help = "Folder to save Phase Maps to.")
...
parser.add_argument('-NPM', help='The number of phase maps to be collected by the camera.',
					type = int, default = 100)
parser.add_argument('-PSA',
					help="Phase shift algorithm to use. f for four point algorithm.n for Novak phase algorithm. c for Carre algorithm.",
					default = 'n')
...
gain = args.gain
NUM_IMAGES = args.NPM
PSA = args.PSA #this variable currently serves no purpuse
Folder_Name = args.f

Data_dir='C:/Users/physics-svc-fulda/Desktop/HetPhaseCam/Phase_Maps'
...
#Removed JPG saving process from original save_acquire on pjflab as of 12/16/2019 (kjl)
		...
		for i in range(NUM_IMAGES):
			if i%4 == 0 and i > 0:
				if PSA == 'f':
					faze = fourpointphase(picList)
				elif PSA == 'n':
					faze = Novak_phase(picList)
				elif PSA =='c':
					faze = carre_phase(picList)
				else:
					faze = Novak_phase_no_mask(picList)
				#saving imgarray
				a = i/4 - 1
				name = Folder_Name + '_%d' % a
				numpy.save('{}/{}'.format(data_path, name), faze)
				del picList[0:4]
